...
useLegacyShortestPath = exist('graphshortestpath', 'file') == 2;
if useLegacyShortestPath
    shortestpath_fn = @(s, t) graphshortestpath(sparse(graphTree), s, t);
else
    shortestpath_graph = digraph(graphTree);
    shortestpath_fn = @(s, t) shortestpath_compat(shortestpath_graph, s, t);
end
...
for i = 1:size(transitionToMatrix,1)
    ...
    for j = 1:length(possibleEnds)
        ...
        if shouldUseTerminalState
            [pathLength, path, pred] = shortestpath_fn(loopStart, possibleEnds(j));
            
            [pathLength1, path1, pred1] = shortestpath_fn(loopStart, terminalStateID);
            [pathLength2, path2, pred2] = shortestpath_fn(terminalStateID, possibleEnds(j));
...
function [UIData] = drawLoops(app, makeButtons, isSelected, seed, maxClusters, loopClusterIDs, finalDynamicsStream, pcaBasis, allLoops, uniqueLoops, clusterMeansPCA, loopBandwidth, mergedLoopIDs, TOTAL_LOOPS, clusterIDs, loopCorrelationMatrix, stateSimilarities, loopWeight, loopLengths, clusterMeans, countClusters)
    ...
    FIG_BASE = 450;
    UIData.f = figure(FIG_BASE + 5);
    clf;
    if isSelected == 0
        drawnow;
    end
    if exist('sgtitle', 'file') == 2
        sgtitle('Loop cluster overview (PCA space)');
    end
    for l = 1:maxClusters
...
function [dist, path, pred] = shortestpath_compat(G, s, t)
% Wrapper to match graphshortestpath output order.
