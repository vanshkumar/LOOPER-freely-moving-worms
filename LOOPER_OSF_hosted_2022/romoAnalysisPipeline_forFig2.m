                ...
                for dec = 0:1
                    ...
                    if ~isempty(f1onlyLower) || ~isempty(f1onlyHigher)
                        if(ismember(fr, f1onlyLower) && dec==1 || ismember(fr, f1onlyHigher) && dec==0)
                            firingRatesAverage(unitCounter, fr, dec+1, :) = nan(size(firingRatesAverage,4),1);
                            continue
                        end
                    end
...
spikeRasters = spikeRasters(1:unitCounter, 1:length(f1stimulationset), ...
                            1:2, 1:timepointCount, 1:max(trialNum(:)));
...
spikeRasters_size = size(spikeRasters);
spikeRasters_sparse = sparse(spikeRasters(:));
...
save([maindir outputFileName], 'firingRates_sparse', 'firingRates_size', ...
                     'firingRatesAverage', 'spikeRasters_size', 'spikeRasters_sparse', 'trialNum', ...
