...
% Optimized to non boostrapped data -- comparable to below on accuracy?
% pcCount = 52;
% epsilon = 50;
% t = 30;
% ks = 2;
...
% Optimized to bootstrapped data -- gives imaginary values?
% pcCount = 43;
% epsilon = 27;
% t = 10;
% ks = 8;
...
[pcaBasis, ~, ~, ~, dataExplained] = pca(dimensionalityStream);
...
save('monkeyHMM.mat', 'allPercents', 'CPT1s', 'covs', 'allMeans', 'CPT3s', 'streamSTD', 'streamMean')
...
oneTrialLength = size(saveData.AsymmetricMap,1) / 60;

useTrials = [1 2 3 31 32 33];
useIndices = [];
for i = 1:length(useTrials)
    useIndices = [useIndices (useTrials(i)-1) * oneTrialLength + (1:oneTrialLength)];
end

mapData = saveData.AsymmetricMap(useIndices(1+4:end),useIndices(1:end-4));

figureHandle = figure(1);
figureHandle.Renderer='Painters';
clf;
colormap(parula(256));
imagesc(sqrt(mapData))
caxis([0 0.1]);
...
% 
% thisStart = squeeze(trialData{plotTrial}(:,startTimes));
% thisTarget = squeeze(trialData{plotTrial}(:,useTimes));
% thisSimulated = squeeze(reconstructedStream(:,plotTrial,:, :));
% 
...
% 
% for i = 1:length(plotNeurons)
%     meanSimulated = nanmean(thisSimulated(:,1:SIM_TIME,:),3);
%     stdSimulated = nanstd(thisSimulated(:,1:SIM_TIME,:),[],3);
%     
